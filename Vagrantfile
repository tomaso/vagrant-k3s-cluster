...
Vagrant.configure("2") do |config|
  ...
  (1..MASTER_COUNT).each do |i|
    config.vm.define "kubemaster#{i}" do |kubemasters|
      ...
      kubemasters.vm.network  :private_network, ip: "192.168.201.#{i+10}"
  ...
  (1..NODE_COUNT).each do |i|
    config.vm.define "kubenode#{i}" do |kubenodes|
      ...
      kubenodes.vm.network  :private_network, ip: "192.168.201.#{i+20}"
  ...
  config.vm.define "front_lb" do |traefik|
      ...
      traefik.vm.network  :private_network, ip: "192.168.201.30"   
